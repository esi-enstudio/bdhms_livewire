@props(['image' => null,'title','subtitle','showUrl' => '#', 'deleteId' => null, 'isCheckbox' => false])

<!-- Card -->
<div {{ $attributes->merge(['class' => 'group flex flex-col h-full bg-white border border-gray-200 shadow-sm rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-neutral-700/70"']) }}>
    <div class="overflow-hidden h-52 flex flex-col justify-center items-center bg-blue-600 rounded-t-xl">
        @if($image)
            <img class="w-full" src="{{ Storage::url($image) }}" alt="{{ $title }}">
        @else
    ...
    <div class="p-4 md:p-6">
        ...
        <div class="mt-3 text-gray-500 dark:text-neutral-500">
            {{ $slot }}
        </div>
    </div>
    <div class="mt-auto flex border-t border-gray-200 divide-x divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
        <a class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-es-xl bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800 dark:focus:bg-neutral-800" href="{{ $showUrl }}" wire:navigate>
            Detail's
        </a>

        <button type="button" wire:confirm="Are you sure you want to delete this record?" wire:click="delete({{ $deleteId }})" class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-ee-xl bg-white text-gray-800 shadow-sm hover:text-white
         hover:bg-red-500 focus:outline-none focus:bg-red-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-red-800 dark:focus:bg-red-800">
            Delete
        </button>
    </div>
    ...
    @if($isCheckbox)
        ...
        <div class="flex absolute top-2 left-2">
            <input wire:model.live="selectedUsers" value="{{ $deleteId }}" type="checkbox" class="shrink-0 mt-0.5 border-gray-200 rounded text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800" id="hs-default-checkbox">
        </div>
        <!-- Checkbox end -->
    @endif
</div>
